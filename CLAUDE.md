...
- **路由重導向端點混淆（外部審查 2026-09-02，嚴重）**：Werkzeug 預設開啟 `merge_slashes`，含連續斜線（`//`）的路徑會被自動 308 重導向到合併後的路徑——**308 保留原始 method 跟 body**，且授權檢查發生在重導向後的全新 request（client 收到 308 後用新 URL 重新發請求，走的是新 URL 命中的端點自己的裝飾器，不是原端點的）。已重現真實案例：`PUT /api/admin/departments/<空dept_id>/reset-password` 被合併成 `PUT /api/admin/departments/reset-password`，命中 `rename_department()`（`/api/admin/departments/<dept_id>`），`dept_id` 的值變成字面字串 `"reset-password"`，若呼叫端 body 剛好帶 `name` 欄位會真的執行改名——這次兩端點權限層級剛好相同（都是 `superadmin_required`）不構成漏洞，是僥倖不是安全。修法（`app.py` `create_app()` 最前面）：`app.url_map.merge_slashes = False` + `before_request` 攔截路徑含 `//` 直接 404，兩者是不同防線，只做前者防不了「路徑段整段消失但前面只有單一斜線」的變體（例如 `POST /api/alarms/` 會落入 `static_url_path=""` 的萬用靜態路由）。
  - **這個防線只擋「自動重導向」跟「明確空段」，防不了「動態參數值剛好等於另一條路由的字面量路徑段」這種更根本的碰撞**——例如 `/api/admin/departments/<dept_id>/actions` 這種「動態段+字面量後綴」的路徑形狀，天生跟 `/api/admin/departments/<dept_id>`（單一動態段）同構：`dept_id` 消失後 collapse 成 `/api/admin/departments/actions`，剛好符合後者的路由格式。這不是端點命名選得不好，是**任何**掛在「動態段+字面量後綴」形狀下的新端點都會跟同一路徑前綴下的「單一動態段」路由碰撞。正確設計原則：動態參數放路徑**最後一段**，且前綴字面量要跟其他路由的固定前綴**不重疊**（例如把 `/api/admin/departments/<dept_id>/actions` 改成 `/api/admin/department-actions/<dept_id>`——`department-actions` 跟 `departments` 是不同字串，dept_id 消失後不會撞到任何路由）。
  - 用 `scripts/check_route_collisions.py` 掃描全部路由找「去掉一段動態參數後會不會意外命中另一條路由」，涵蓋未來新增的端點，不用每次新增路由都手動比對——掃出來的碰撞不代表都是漏洞（命中 Flask 靜態檔案萬用路由 `static` 的情況，實際請求會依檔案是否存在而 404，不構成資訊洩漏或邏輯繞過；命中「權限要求更高」端點的情況，方向是變嚴格不是變寬鬆），但命中「權限要求更低」或「業務邏輯不同但同樣可執行」的端點就是真正的風險，要逐一判讀不能只看數字。
  - 順帶：`DepartmentStore`（`storage.py`）是唯一沒有走 `_use_supabase()` 分支判斷的 Store 類別——`ALARM_DATA_DIR` 測試隔離對它完全無效，任何呼叫都直接打 `.env` 載入的真實 `SUPABASE_URL`（若忘記在本機腳本裡明確覆寫成假值）。修法：`_urlopen()` 單一入口 + `_guard_production_write()`，本機執行對正式環境的寫入方法一律 `RuntimeError` 擋下，除非明確設 `ALLOW_LOCAL_PRODUCTION_WRITE=1`。任何人要寫本機腳本重現部門相關問題，都要先確認這件事，不能假設「沒設 `ALLOW_LOCAL_PRODUCTION_WRITE` 以外的東西就是安全的」。**這層護欄攔在 `_urlopen()` 這個單一入口，任何新增的 HTTP 呼叫一律要走 `_urlopen()`，不要直接用 `urllib.request.urlopen()` 或引入新的 HTTP client（`requests`/`httpx` 等）——繞過這個入口就會繞過護欄本身，不是護欄失效，是根本沒經過它。**
