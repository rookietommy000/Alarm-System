...
**兩層安全網的定位（專家原話）**：第 2 項（`rollback_stage3.sql`）與第 5 項（交易包裹）是這次操作**真正的安全網**——精準、快速、針對「這一步做錯了」的情況，日常操作出錯走的是這兩項。第 3、4 項（`pg_dump`、Dashboard 備份）是**「整個專案出事」時的保險**——範圍更大、恢復更慢，用於災難情境（例如誤刪整個 schema、Supabase 專案本身出問題），只有前兩項都失效或問題超出這次遷移範圍時才會用到。**這個優先順序不代表 `pg_dump` 可以省略**——它排在 2、5 之後是「最後一道」，但不是「可有可無的一道」，六步驟仍要全部照做完才能進入下一階段。
