...
🟢 **階段 1-5 已完成、已 commit、已 push、已在正式環境驗證，含一次重大方向轉折（審核路徑停用）與三個修復。**

**下一個 session 接手時，從這裡開始：** 階段 5 全部完成並上線，下一步是階段 6（缺處置清單）。審核機制（一般使用者提交建議、管理員審核）已停用，改為所有登入者直接編輯 + 變更歷史追溯，見下方「審核路徑停用（決策記錄）」。

**這一輪的 commit（由舊到新）**：
- `c0b6b80` 階段 5 第一版：一般使用者建議＋待審表＋後台審核（完整實作，含 `dashboard.html` 待審清單）
- `b7323fa` 舊 Render 服務清理（意外發現）
- `7eb73e8` 移除參考清單功能、新增 `AUTH_FLOW.md`
- `ee85162` **審核路徑停用決策**：`PUT .../local` 改 `login_required`，新增 `history` 端點，前台改顯示「最後修改」+ 變更紀錄，移除待審清單 UI，`alarm_suggestions` 保留不刪，加 `before_request` 靜態路徑保護
- `68d50c8` 修復：`toggleLocalHistory` 在 `whoami.department` 為 `null`（超管）時組出 `"null"` 字面路徑
- `7300039` 修復：`openLocalSolutionEditor` 同類問題（塌陷成空字串），超管前台隱藏編輯按鈕
- `42c7186` 統一四處 `abort(404)` 訊息為 `NOT_FOUND_MSG`，新增 `tests/conftest.py` 收斂測試 fixture

**已知未完成、待處理項目**：
1. 🟡 `review_note`（後台退回建議時的理由欄位）是否強制填——這題已隨審核路徑停用而**失去意義**，`alarm_suggestions` 相關 UI 已整個移除，不需要再處理
2. 🟡 曾問過專家「`docs/index.html` 死連結要不要現在改」——**已處理**（`b7323fa` 已更新網址），此項可視為完成
3. ⬜ 階段 6（缺處置清單）尚未開始
4. ⬜ 階段 7（AI 第一關分級）尚未開始，且明確排在階段 6 之後、不要提前做（第七節警告）
...
## 七、🟡 執行順序（1-5 完成，6-7 未開始）

每個階段都能獨立部署並單獨驗證，**不需要維護窗口**。

| 階段 | 內容 | 產生的價值 | 狀態 |
|---|---|---|---|
| 1 | 加四個欄位 ＋ 建 `alarm_suggestions` 表（第三輪審查後：不建 view，見 4.5 節） | 零行為變更 | ✅ 完成 |
| 2 | `PUT .../local` 端點 ＋ 白名單 ＋ 稽核 | 後端就緒 | ✅ 完成 |
| 3 | 前台詳情卡片顯示兩層 | **既有知識可見** | ✅ 完成 |
| 4 | 前台管理員編輯入口 ＋ 預填情境 | **知識開始累積** | ✅ 完成 |
| 5 | 現場方案直接編輯（所有登入者）＋ 變更歷史追溯（原規劃審核機制，已完整驗證後停用，見狀態區塊決策記錄） | 全員可貢獻 | ✅ 完成 |
| 6 | 缺處置清單（`GET /api/alarms?missing_local=true` 篩選參數，不做排序，見 4.5 節） | 補資料有優先序 | ⬜ 未開始 |
| 7 | AI 第一關分級 ＋ 差異摘要 | 降低審核負擔 | ⬜ **暫緩，待個人帳號功能完成後重新評估**（見下方修正） |

**🔴【2026-08-24 修正】階段 7 的狀態說明已過時，原文「依賴階段 6」不再成立**：原本「不要先做 AI 審核，會變成有審核機制但沒東西可審」這個警告，前提是「審核路徑存在、只是還沒東西可審」。但審核路徑（`alarm_suggestions` 待審清單）已經在規劃二完整驗證後**主動停用**（見狀態區塊「審核路徑停用（決策記錄）」）——現在是空表、無前端入口，不會有任何東西流進待審清單，AI 審核沒有審核對象可以掛，跟階段 6 有沒有做完全無關。

階段 7 正確的狀態是**跟 `alarm_suggestions` 一樣，暫緩到個人帳號功能完成、重新評估是否啟用審核路徑之後**，而不是「排在階段 6 後面等著」。下一個接手的人不要再以為做完階段 6 就該接著做階段 7。
...
⚠️ **不要先做 AI 審核**——原始理由已過時（見上方修正），但結論不變：階段 7 目前沒有審核對象、也還沒有個人帳號可以支撐審核者可信度判斷，兩個理由都指向同一個結論——暫緩。
...
- ✅ **【2026-08-24 已查證並排除】缺處置的警報中，有多少是「原廠文件其實有、但當初沒匯進來」？** 用真實 Supabase 資料查證 `mf4d` 部門：總數 1759 筆，缺 `local_solution` 1754 筆（幾乎全部），其中 `solution`（原廠處置）也是空的有 993 筆、`solution` 已有內容的有 761 筆（43.4%）。**使用者確認 993 筆是同仁尚未提供資料，不是系統匯入時漏掉**——不存在「原廠文件其實有但沒匯進來」這個缺口，批次補匯入這條路線不成立，不需要另外規劃匯入流程。這 993 筆的正確去向就是本功能原本的設計：等現場累積經驗後由登入者直接補 `local_solution`。
- 差異較大的（AI 標 🔴）是否需要更資深的人確認？（與審核者角色待辦同一類，暫不處理，且階段 7 本身已暫緩，見第七節修正）

**連帶結論：階段 6 的清單不需要區分「漏匯入」與「原廠真的沒有」兩類特殊處理**，只需要沿用既有的「有無原廠 `solution`」判斷（跟 5.1 節詳情卡片的四分支邏輯共用同一個判準）：761 筆有原廠內容的顯示「尚未補充現場方案」，993 筆兩者皆空的顯示「這筆還沒有任何處置方式」。不需要新增欄位、不需要後端做額外分類邏輯，`missing_local=true` 篩選出的清單本身已經帶 `solution` 欄位，前端渲染時判斷即可。

**「PDF／Word → 標準 CSV 抽取流程」待辦**（`PLAN_department_isolation.md` 佔位章節）：因批次匯入路線已排除，這個抽取流程不再是階段 6 的前置依賴，維持佔位狀態即可，不用因為這次查證結果而補實質內容。
